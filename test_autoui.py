...
def copy_share_dir(src_path):
    ...
    os.makedirs(rf'{dash_share}\Logs')
    run_cmd(rf'net share Logs={dash_share}\Logs /grant:DashService,Full /grant:DashAdmin,Read')
    dash_content = rf'{cur_path}\DashContent'
    ...
    run_cmd(
        rf'net share DashContent={dash_share}\DashContent /grant:DashAdmin,Full /grant:DashService,Read'  # noqa: E501
    )
    return
...
def whether_to_install_docker():
    if (
        not run_cmd(
            [
                'powershell.exe',
                'Get-WindowsOptionalFeature -Online | Where-Object { $_.State -eq "Enabled" -and $_.FeatureName -eq "Microsoft-Hyper-V" }',  # noqa: E501
            ]
        ).stdout
        == ''
    ):
        run_cmd(
            [
                'powershell.exe',
                'Enable-WindowsOptionalFeature -Online -FeatureName $("Microsoft-Hyper-V", "Containers") -All -Norestart',  # noqa: E501
            ]
        )
...
def do_key_input(msg, clip_board_mode=True, key_sleep=0.0):
    if clip_board_mode:  # 剪贴板方式
        win32clipboard.OpenClipboard()
        win32clipboard.EmptyClipboard()
        win32clipboard.SetClipboardText(msg, win32con.CF_TEXT)
...
def install_dash():
    ...
    time.sleep(2)
    run_cmd(
        f'netsh interface ipv4 set address "{network_name}" static 192.168.1.2 255.255.255.0 192.168.1.2'  # noqa: E501
    )

    subprocess.Popen(r'cmd.exe /c' + f'{cur_path}/install.cmd')
    wait_for_window(r'C:\Windows\system32\cmd.exe', max_time=3)  # 最多等3秒钟
